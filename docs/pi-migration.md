...
| `bridge` | The extension uses capability-shaped tools, commands, lifecycle events, notifications, or similar surfaces suitable for a compatibility process. The compatibility process does not ship yet. |
...
will or will not exercise an effect at runtime. A future bridge would still run
...
The shipped dry run is the inventory front end for this stage. A later apply
stage can copy compatible skills/prompts, generate Ygg manifests, transform
known configuration, and cache intermediate results by source and lock hash.
Those operations should remain deterministic and model-free.

### Compatibility process

A future `ygg-pi-compat` executable extension can host a deliberately bounded
subset of Pi's `ExtensionAPI` and translate it onto Ygg API `0.2`. Bridged Pi
extensions that use an in-process event bus, `globalThis`, or shared registries
must live in the same compatibility process; Ygg should make one JSON-RPC call
per subscribed lifecycle event and fan out locally.
