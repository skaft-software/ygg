...
the local host has an executable V2 runtime. Offline or incomplete metadata is
conservative: model names and OAuth plans never imply Ultra, Responses Lite, or
delegation.
...
root, depth two, and sixteen total agents over its lifetime. Children have
isolated durable sessions and inherit the root's approved tools, sandbox, model,
reasoning, compaction, completion, output, retry, turn, and cost policies.
Cancellation and interruption propagate to descendants.
...
cancels the team and rejects further work.
