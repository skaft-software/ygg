...
- Typed semantic rich-text documents, CommonMark/GFM parsing, unified diffs, safe links, and copy/plain encoders.
- Stable-prefix, bounded-tail streaming Markdown with arbitrary UTF-8 byte ingestion and cached live layout.
- Optional syntect highlighting mapped to semantic theme roles with a bounded renderer-local cache.
- Central terminal capabilities, explicit overrides, color quantization, conservative glyph sets, and deterministic plain mode.
- Grapheme/display-cell-correct wrapping, clipping, tables, lists, quotes, code blocks, input, and editor cursor handling.
- Generation-safe `LiveRegion` nodes and chronological noninteractive events.
- Hostile-control sanitization, width/capability goldens, adversarial streaming tests, examples, benchmark smoke harness, and Ygg migration guidance.

### Changed

- `widgets::Markdown` now retains and renders a semantic document; the legacy constructor remains source-compatible.
- Terminal image, theme, TUI, and rich rendering share one `TerminalCapabilities` model.
- TUI synchronized output is capability-gated, pure appends are rendered, resize redraws home correctly, and stop restores terminal state.
