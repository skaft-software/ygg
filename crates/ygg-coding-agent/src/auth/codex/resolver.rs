...
use super::{now_unix, oauth, random_hex, REFRESH_SKEW_SECS, TOKEN_URL};
...
pub struct CodexResolver {
    ...
    token_url: String,
    /// Stable for the process lifetime; sent as `session-id`.
    session_id: String,
...
impl CredentialResolver for CodexResolver {
    async fn resolve(&self) -> Result<ResolvedCredential, AuthError> {
        ...
        extra_headers.insert(http::HeaderName::from_static("session-id"), session);
...
mod tests {
    ...
    async fn resolves_unexpired_token_without_network() {
        ...
        assert!(cred.extra_headers.get("session-id").is_some());
    }
