...
impl Tool for SearchTool {
    fn definition(&self) -> ToolDef {
        ToolDef {
            ...
            description: "Search local file contents. Relative paths resolve from the workspace; \
                          trusted-local hosts also accept absolute and `~/` paths. Matches are \
                          literal by default; set mode=regex for regular expressions. Returns \
                          `path:line  text` lines with a match count and truncation flag."
                .to_string(),
