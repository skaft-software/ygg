...
impl Tool for ReadTool {
    fn definition(&self) -> ToolDef {
        ToolDef {
            name: "read".to_string(),
            description: "Read a local file with line numbers. Prefer paths relative to the workspace \
                          (for example `src/main.rs`); trusted-local hosts also accept absolute and `~/` \
                          paths for intentional external files. Returns a header `path:start-end/total \
                          hash=…` (the hash covers the whole file and is used for edit's expected_hash), \
                          numbered lines, and `next_offset=N` when more lines remain."
                .to_string(),
    ...
    async fn execute(
        ...
        let bytes = tokio::task::spawn_blocking(move || {
            read_regular_file_bounded(&read_path, MAX_FILE_BYTES)
        })
        .await
