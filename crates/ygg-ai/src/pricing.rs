...
pub struct Cost {
    ...
    pub input: u64,
    /// Cost for generated output tokens.
    pub output: u64,
...
pub fn cost_of(pricing: &Pricing, usage: &Usage) -> Result<Cost, PricingError> {
    ...
    // 3. Compute costs using u128 intermediate calculations
    let input_cost_u128 = (usage.input_tokens as u128 * rates.input.0 as u128) / 1_000_000;
    let cache_read_cost_u128 =
        (usage.cache_read_tokens as u128 * rates.cache_read.0 as u128) / 1_000_000;

    // Cache write cost: 5m and 1h separate
    let cache_write_5m_tokens = usage.cache_write_tokens - usage.cache_write_1h_tokens;
    let cache_write_5m_cost_u128 =
        (cache_write_5m_tokens as u128 * rates.cache_write_5m.0 as u128) / 1_000_000;

    let active_cache_write_1h_rate = rates.cache_write_1h.unwrap_or(rates.cache_write_5m);
    let cache_write_1h_cost_u128 =
        (usage.cache_write_1h_tokens as u128 * active_cache_write_1h_rate.0 as u128) / 1_000_000;

    let cache_write_cost_u128 = cache_write_5m_cost_u128
        .checked_add(cache_write_1h_cost_u128)
        .ok_or(PricingError::ArithmeticOverflow)?;

    // Output cost: reasoning and regular output separate
    let output_standard_tokens = usage.output_tokens - usage.reasoning_tokens;
    let output_standard_cost_u128 =
        (output_standard_tokens as u128 * rates.output.0 as u128) / 1_000_000;

    let active_reasoning_rate = rates.reasoning.unwrap_or(rates.output);
    let reasoning_cost_u128 =
        (usage.reasoning_tokens as u128 * active_reasoning_rate.0 as u128) / 1_000_000;

    let output_cost_u128 = output_standard_cost_u128
        .checked_add(reasoning_cost_u128)
        .ok_or(PricingError::ArithmeticOverflow)?;

    let total_cost_u128 = input_cost_u128
        .checked_add(cache_read_cost_u128)
        .and_then(|v| v.checked_add(cache_write_cost_u128))
        .and_then(|v| v.checked_add(output_cost_u128))
        .ok_or(PricingError::ArithmeticOverflow)?;

    // 4. Safely downcast to u64
    let input = u64::try_from(input_cost_u128).map_err(|_| PricingError::ArithmeticOverflow)?;
    let cache_read =
        u64::try_from(cache_read_cost_u128).map_err(|_| PricingError::ArithmeticOverflow)?;
    let cache_write =
        u64::try_from(cache_write_cost_u128).map_err(|_| PricingError::ArithmeticOverflow)?;
    let output = u64::try_from(output_cost_u128).map_err(|_| PricingError::ArithmeticOverflow)?;
    let total = u64::try_from(total_cost_u128).map_err(|_| PricingError::ArithmeticOverflow)?;
