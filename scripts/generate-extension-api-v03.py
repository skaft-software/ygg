...
def validate_schema(schema: dict[str, Any]) -> None:
    ...
    for method in schema["methods"]:
        ...
        if method.get("terminal") not in {"initialized", "result_or_error", "shutdown", "original_request_cancelled", "deferred"}:
...
def render_python(schema: dict[str, Any], source_hash: str) -> str:
    ...
    lines.extend([
        ...
        "        if field.get('max_bytes') is not None and len(_utf8_bytes(item, field_name)) > globals()[constant_name(field['max_bytes'])]: raise ContractError('resource_exhausted', f'{name}.{field_name} exceeds byte bound')",
...
def render_typescript_runtime(schema: dict[str, Any], source_hash: str) -> str:
    ...
    output = output.replace("_utf8Bytes(item, field.name).length > globalThis[field.max_bytes.toUpperCase()]", "_utf8Bytes(item, field.name).length > _bound(field.max_bytes)")
    return output
