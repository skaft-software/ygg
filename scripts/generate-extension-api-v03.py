...
def render_rust(schema: dict[str, Any], source_hash: str) -> str:
    ...
    lines.extend([
        ...
        "fn validate_optional_host_offer(values: &BTreeSet<String>, allowed: &[&str], kind: &str) -> Result<(), ContractError> { if let Some(value) = values.iter().find(|value| !allowed.iter().any(|allowed| *allowed == value.as_str())) { return Err(ContractError::named(\"capability_mismatch\", format!(\"host offer optional {kind} {value:?} is not generated by API 0.3\"))); } Ok(()) }",
