...
def render_rust(schema: dict[str, Any], source_hash: str) -> str:
    ...
    lines.extend([
        "",
        # This generated module is rustfmt-skipped; keep independent guard
        # clauses on separate lines so compact code cannot resemble else-if.
        ...
        f"    validate_optional_host_offer(&optional_capabilities, &[{rust_array(optional_capabilities)}], \"capability\")?;",
...
def render_python(schema: dict[str, Any], source_hash: str) -> str:
    ...
    lines.extend([
        ...
        f"    if required != set({required_capabilities!r}) or not optional <= set({optional_capabilities!r}): raise ContractError('capability_mismatch', 'host offer capability sets differ from generated API 0.3 contract')",
        ...
        f"    if required_methods != set({required_methods!r}) or not optional_methods <= set({optional_methods!r}): raise ContractError('capability_mismatch', 'host offer method sets differ from generated API 0.3 contract')",
