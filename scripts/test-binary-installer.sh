...
case "$request" in
    *'"request_id":"installer-probe"'*)
        printf '%s\\n' '{"protocol_version":1,"request_id":"installer-probe","seq":1,"type":"hello","data":{"sdk_version":"0.6.1"}}'
        ;;
    *) exit 2 ;;
esac
''',
...
PY
    {
        ...
        printf '%064d  ./ygg-0.6.1-x86_64-apple-darwin.tar.gz\n' 0
        printf '%064d  ./ygg-0.6.1-x86_64-unknown-linux-gnu.tar.gz\n' 0
...
if [ "${YGG_TEST_HARDLINK_ARCHIVE:-0}" = 1 ] && [ "$name" = ygg-0.6.1-aarch64-apple-darwin.tar.gz ]; then
    ln "$source" "$output"
else
    cp "$source" "$output"
fi
if [ "${YGG_TEST_TAMPER_ARCHIVE:-0}" = 1 ] && [ "$name" = ygg-0.6.1-aarch64-apple-darwin.tar.gz ]; then
...
printf '%s\n' '{"protocol_version":1,"request_id":"installer-probe","command":"hello"}' \
    ...
    | grep -F '"sdk_version":"0.6.1"' >/dev/null
test "$("$positive_home/bin/ygg" --version)" = 'ygg 0.6.1'
...
test "$("$upgrade_home/bin/ygg" --version)" = 'ygg 0.6.1'
printf '%s\n' '{"protocol_version":1,"request_id":"installer-probe","command":"hello"}' \
